...
- Check if a discovery report exists from a previous `/brand-voice-pro:discover-brand` run
- Check `.claude/brand-voice.local.md` for known brand material locations
- Suggest running discovery first: `/brand-voice-pro:discover-brand`
...
2. Start creating content with `/brand-voice-pro:enforce-voice`
