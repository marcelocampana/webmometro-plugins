...
artículo, una página—, su archivo fuente lleva un `estado:` en el frontmatter que hay que mover
**dentro de la misma cadena de cierre**, sin una pregunta aparte. Si no se actualiza ahí, el archivo
queda mintiendo sobre su propio estado y cualquier verificación posterior parte de un dato falso.

**Solo entonces**, lee `cierre-contenido.md`: la escala de estados, por qué `validado` no es un
`aprobado` más fuerte, y qué hacer cuando la tarea modifica una pieza ya validada.
